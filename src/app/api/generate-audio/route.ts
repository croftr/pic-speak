...
export async function POST(request: Request) {
    ...
    const limited = await rateLimit(userId, 'generate-audio', MAX_REQUESTS, WINDOW_MS, true);
    if (limited) {
    ...
    const dailyLimited = await checkDailyLimit(userId, 'generate-audio', MAX_PER_USER_PER_DAY, true);
    if (dailyLimited) {
