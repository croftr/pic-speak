...
export default function HomeClient({ initialSignedIn }: { initialSignedIn: boolean }) {
  ...
  const { isSignedIn: liveSignedIn } = useOfflineAuth();
  // The server already decided which hero variant to render — trusting its
  // answer until clerk-js finishes loading keeps the initial HTML stable
  // (no signed-out → signed-in hero swap, which showed up as CLS).
  const isSignedIn = isLoaded ? liveSignedIn : initialSignedIn;
