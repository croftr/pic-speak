...
function PublicBoardsContent({ initialBoards }: { initialBoards: Board[] }) {
    const searchParams = useSearchParams();
    const creatorFilter = searchParams.get('creator');

    const [publicBoards, setPublicBoards] = useState<Board[]>(initialBoards);
...
export default function PublicBoardsClient({ initialBoards }: { initialBoards: Board[] }) {
    return (
        <main className="min-h-screen py-8 sm:py-16">
            <Suspense fallback={<div className="py-20 text-center text-gray-500">Loading...</div>}>
                <PublicBoardsContent initialBoards={initialBoards} />
            </Suspense>
        </main>
    );
}
