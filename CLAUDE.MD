...
- `public/sw.js` - hand-written service worker. Cache strategies: cache-first for immutable Vercel Blob media and `/_next/static`, network-first with cache fallback for navigations/RSC/API GETs, `public/offline.html` fallback for never-visited pages. Bump `CACHE_VERSION` in `sw.js` when a deploy must flush stale cached pages (media cache is versioned separately and survives bumps — its URLs are immutable).
