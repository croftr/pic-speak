...
- `src/components/lock/` - `LockedBar.tsx` (minimal top bar: board name, card-size toggle, unlock button), `HoldToUnlockButton.tsx` (3s press-and-hold with an SVG progress ring; a short tap just shows a "Hold to unlock" hint), `UnlockQuizDialog.tsx` (simple addition question with decoy answers; 3 wrong attempts closes the dialog and stays locked)
